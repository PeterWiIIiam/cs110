""" This is a functioning Nim game, albeit quite bare-bones. """
...
class Bean(EventHandler):
    ...
    def __init__(self, win, radius, color, bgcolor, location, on):
        ...
        self._on = on                 # True if on, False if off
        
        self._body = Circle(win, self._radius, self._location)
        self._body.set_fill_color(self._color)

        self._body.add_handler(self)  # the Bean is its own handler
...
class Game(object):
    ...
    def __init__(self, win, winheight, bgcolor):
        ...
        for heap in range(3):
            size = heap_sizes[heap]
            this_heap = heap_grid[heap]
            for spot in range(size, len(this_heap)):
                this_heap[spot]._on = False
                this_heap[spot]._body.set_fill_color(self._bgcolor)
                this_heap[spot]._body.set_border_color(self._bgcolor)
